...
- **固定变量处理**：如果某些变量的上下界相等（例如 $x_j = 0$），直接将这些变量固定，并从问题中删除对应的列；同时将其对约束右端项与目标函数常数项的贡献吸收到 $b$ 与常数项中。
...
- **构造初始单纯形表**：在“标准形最大化”问题下，对人工变量在目标中施加一个负的大系数（实现中取 $-M$），相当于强烈惩罚人工变量取正值；随后计算初始约化成本行与目标值 $z$，并组装单纯形表。
...
为评估求解器性能，我们编写了基准测试脚本 `lp_solver/examples/benchmark.py`。该程序的核心功能如下：

1. **生成测试问题**：自动生成不同规模（约束和变量数量）的随机线性规划问题；由于随机性与数值问题影响，生成的问题不保证全部可行/可解，因此需要统计成功率。
...
结合基准测试表格与图表，对结果做如下总结：

1. **正确性与稳定性**：在求解成功的样本上，本求解器的结果与 SciPy 基本一致；但在较大规模下成功率有所下降（例如表中最大规模自研为 75%，SciPy 为 100%），说明当前实现仍存在数值稳定性或鲁棒性不足的问题。
2. **性能瓶颈明确**： 从模块耗时分解图可以看出，**`模块3：单纯形迭代`** 在大规模问题中占据主要耗时，其实现效率直接决定了整个求解器的性能上限。
3. **与成熟求解器的效率差距**： 从基准测试数据可以看出，本求解器与 SciPy 的耗时差距会随规模增大而明显扩大。主要原因在于算法与工程实现差异：本实现采用标准单纯形表法，而 SciPy 等库通常采用修正单纯形/内点法等更优化的实现，并配合数值线性代数优化。
